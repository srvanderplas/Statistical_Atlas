...
\svp{Need to add in plate-32-redo with no frame - this sentance doesn't make sense without that.}
...
There are also quite a few very extreme differences between estimated and actual size of areas apparent, particularly for small areas in mosaic plots and un-framed pie charts.
...
As a measure of the error we use the log ratio between the estimated and the actual size of the piece. A positive log ratio means that the participant  overestimated the size of the piece in his or her estimate. Similarly, a negative log ratio corresponds to a visual underestimation of the piece.\svp{This should change, shouldn't it?}

\svp{Need to discuss excluding the errors which are more than 5x higher/lower.}
...
This approach brings us back directly to the psychophysics equation
$A_{est} \propto A_{act}^\beta$ by \citet{size},
which can be written as the linear dependency $\log A_{est} = \alpha + \beta \log A_{act}$.
Figure~\ref{fig:model} shows a scatterplots of log estimated versus log actual sizes of areas, with simple linear regressions summarising the relationship between estimated and actual size for each of the plot types and each of the types of pieces.
%of the difference in estimated and actual size of the area in the plot colored by different types of areas. We can see that for inside pieces of unframed and unframed designs participants have a tendency to overestimate the size of small areas and underestimate the size of the larger areas. For framed designs the error made in the visual estimation is much larger overall and depends on the location of the estimated area. Areas on the inside are overestimated, while areas of the frame are grossly underestimated. We can model these differences in estimated and actual area size as follows.
...
\begin{itemize}
\item $\beta_1, \beta_2, \beta_3$ represent perceptual biases for area estimation in unframed (regular) mosaic, pie, and spine charts. A $\beta$ value of one would indicate no bias; $\beta > 1$ indicate over-estimates, and $\beta < 1$ indicate under-estimates. Spine plots ($\hat\beta_3$ = \Sexpr{round(xtab[14,4], 3)}) are less prone to this bias; Mosaic plots ($\hat\beta_1$ = \Sexpr{round(xtab[2,4], 3)}) are the worst. The two-dimensional nature of area assessment in mosaic plots results in poorer estimates, which is consistent with our original hypothesis.
\item Estimation of the inside portion of framed plots is not significantly different from estimation of portions of unframed plots in any of the plot types. Both $\alpha_{\cdot 2}$ and $\beta_{\cdot 2}$ are not significantly different from the unframed plot estimates.
\item Estimation of the framed portion of the plot is significantly worse than estimation of the same plot area in the unframed version of the plot for mosaic and pie charts, but not for spine plots.
\end{itemize}
