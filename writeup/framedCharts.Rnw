...
The other two areas were chosen such that in each plot a small, a medium, and a large area were selected for estimation. To be able to more efficiently estimate variability and ensure comparability, three states were chosen for each plot type and rendered  both with a frame and unframed, for a total of 18 plots. 
These 18 plots were assembled in three sets such that all combinations of plots are included exactly once and no state is included more than once. Each participant then got to evaluate one of these sets.
