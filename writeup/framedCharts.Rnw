...
createMosaics <- function(data = occ3, state_name = "") {
  ...
  plot1 <- ggp_df %>% ggplot() +
    geom_rect(xmin=0, xmax=1, ymin=0, ymax=1, fill="grey60", colour="grey30", size=.5) +
...
For all six plots participants were asked to estimate the size of the frame in framed designs and the corresponding integrated area in designs without a frame.
The other two areas were chosen such that in each plot a small, a medium, and a large area were selected for estimation. To be able to more efficiently estimate variability and ensure comparability, three states were chosen for each plot type and rendered with and without a frame, for a total of 18 plots.
These 18 plots were assembled in three sets such that all combinations of plots were included exactly once and no state was included more than once. Each participant evaluated one set of plots.
