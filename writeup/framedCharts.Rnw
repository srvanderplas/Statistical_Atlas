...
Let $r_{ijk\ell}$ be the ratio of estimated and actual size of the questioned area for plot type $i$ (with 1 = mosaic, 2 = pie, and 3 = spine), piece location $j$ (with 1 = piece in unframed plot, 2 = inside piece of a framed plot, and 3 = frame piece of a framed plot) and response $k$ where $k = 1, ..., n_{ij}$, where $n_{ij}$ is the number of evaluations for plot-frame location combination $ij$. We then model the log ratio $\log r_{ijk\ell}$. Results in the table are from the log ratio. Each combinations has at least 25 responses from reddit users and 50 responses from amazon turk participants (see figure~\ref{fig:demographics} for an overview).

The model is written in the form of regressions lines of the log ratio $\log r$ by actual size $p$, with the unframed version of each of the types as base line. The other parameters can then be written as contrasts to the base line, producing an easily interpretable results.

We model $r_{ijk\ell}$ as Model (1)
\begin{eqnarray*}
\log r_{ijk\ell} &=& \alpha_{i} + \beta_{i} \cdot p  + \left(\alpha_{ij} + \beta_{ij} \cdot p\right) + e_{ijk} + u_\ell,
\end{eqnarray*}
where
\begin{itemize}
...
\item[$\beta_i$] can be interpreted as the average multiplicative change in the ratio of estimated and actual size of a piece in an unframed design of plot type $i$ ($i = 1, 2, 3$) when the actual size increase by 1 percentage point,
