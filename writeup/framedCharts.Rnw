...
percentages <- read.csv(file.path(datapath, "study-results.csv"))
...
error2b <- lm(diff.error ~ Type-1 + (frameframe*perc):Type, data = percentages)
@
...
Let $d_{ijk}$ be the difference between estimated and observed difference of the questioned area, where 
\begin{itemize}
\item[$i$] corresponds to the plot type, with 1 = mosaic, 2 = pie, and 3 = spine, 
\item[$j$] is the piece location, with 1 = piece in unframed plot, 2 = inside piece of a framed plot, and 3 = frame piece of a framed plot.
\item[$k$] is the number of responses for each combination of $i$ and $j$. Each combinations has at least 25 responses from reddit users and 50 responses from amazon turk participants. See figure~\ref{fig:demographics} for an overview. 
\end{itemize}

\hh{the idea is to write the model in form of intercepts and slopes by actual size, with the unframed version of each of the types as base line. The other parameters can then be written as contrasts to the base line and be interpreted nicely.}

We model $d_{ijk}$ as 
\begin{equation}\nolabel
d_{ijk} &=& a_{i} + b_{i} \cdot p  + \left(a_{ij} + b_{ij} \cdot p\right) + \epsilon_{ijk},\\
\end{equation}
