...
Three times in the past, the US Census Bureau published a Statistical Atlas to map the state of the Union based on data collected in the 9th, 10th, and 11th US census (in 1870, 1880, and 1890). Each of these atlases is a masterful combination of art, science, and technology. The ninth census, supervised by Francis A. Walker, assessed a country of about 38.5 million people. The Statistical Atlas of 1870 is a graphical compendium of information from the 9th census, laid out over more than 100 pages. Most of the illustrations are overlaid maps (38 of 107 pages), but some consist of more abstract and, at that time, novel statistical plots, including age pyramids (bilateral frequency charts) and ring charts to show population growth. This paper addresses two types of charts from the 1870 Statistical Atlas which were presented as small multiples; that is, a separate chart was shown for each state and territory.
...
\begin{figure}
  ...
  \includegraphics[width=0.4\textwidth,valign=c]{images/occupations-us.png}
  \label{fig:us}
  }
  \hfill
  \subfloat[Miniature of plate \#32 of the Statistical Atlas.]{
\includegraphics[width=.54\textwidth,valign=c]{images/ca000082.jpg}
  \label{fig:plate32}
  }
...
Let $E_{ijk\ell}$ be the estimated size of the area for a piece in plot type $i$ (with 1 = mosaic, 2 = pie, and 3 = spine),  location $j$ (with 1 = piece in unframed plot, 2 = inside piece of a framed plot, and 3 = frame piece of a framed plot), response $k$ where $k = 1, ..., n_{ij}$, where $n_{ij}$ is the number of evaluations for plot-frame location combination $ij$ by one of the participants $\ell$, $\ell = 1, ..., 95$. Let  $A_{ij}$ be the corresponding actual size of the piece.
<<demographics2, fig.width = '.7\\textwidth', fig.height = 4, fig.cap="Breakdown of the number of evaluations by source (reddit or amazon) for all combinations of plot type and piece locations.">>=
...
\begin{itemize}
\item $\alpha_1 + \beta_1\log(A)$, $\alpha_2 + \beta_2\log(A)$, $\alpha_3 + \beta_3\log(A)$ represent estimates for a participant's assessment of an area of size $\log(A)$  in unframed (regular) mosaic, pie, and spine charts. Values of $\alpha_i = 0$ and $\beta_i=1$ indicate no bias; estimates with $\alpha_i + \beta_i\log(A) < \log(A)$ indicate a visual under-estimation of the area, while estimates with $\alpha_i + \beta_i\log(A) > \log(A)$  indicate an over-estimate. All parameter estimates $\beta_i$, $i=1,2,3$, are smaller than 1, indicating that participants tend to underestimate the size of large areas $A$. For small areas $A$ this effect is reversed: all parameter estimates $\alpha_i$, $i=1,2,3$ are positive, indicating that participants tend to overestimated the size of small areas. Spine plots ($\hat\alpha_3$ = \Sexpr{round(xtab[13,4], 3)}, $\hat\beta_3$ = \Sexpr{round(xtab[14,4], 3)}) are less prone to these biases, in particular $\alpha_1$ is not significantly different from zero; Mosaic plots ($\hat\alpha_1$ = \Sexpr{round(xtab[1,4], 3)}, $\hat\beta_1$ = \Sexpr{round(xtab[2,4], 3)}) on the other hand exhibit the worst biases. The two-dimensional nature of area assessment in mosaic plots results in poorer estimates, which is consistent with our original hypothesis.
...
\begin{figure}
  \subfloat[Variability is lower in responses from participants recruited from reddit compared with participants recruited from Amazon Turk.]{
