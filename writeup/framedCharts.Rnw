...
\hh{
Compared to demographics of the general US population \citep[as published in][]{census2016} we see in \autoref{fig:demographics} that survey participants are generally younger and more highly educated. This effect is even more pronounced amoung participants recruited from reddit than amazon turkers. XXX gender
}
...
While aesthetically appealing, framed charts are an inferior choice if the goal is to effectively communicate information. The Census Bureau did an incredible job at collecting, tabulating, visualizing, and maintaining the data presented in the Statistical Atlas. Reproducibility has become a more discussed topic in science lately, but few researchers attempt to preserve their data and methods in such a way that the results could be precisely duplicated 150 years later. The Census Bureau (in conjunction with the Library of Congress) has managed to make it fairly easy for researchers to access the data necessary to conduct this study online.

In addition to the technical feat of reproducibility, the Statistical Atlas represented a state-sponsored foray into the development and use of new types of charts. While framed plots are aesthetically appealing, they are not effective at communicating numerical information accurately. With 107 pages of charts, and several relatively new types of plots, it is not surprising that some of these experiments fell flat. In addition, due to the limitations of printing technology at the time, some of the design choices are pragmatic, as well as aesthetic.
