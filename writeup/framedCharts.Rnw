...
<<state-church-plot, dependson="code", echo = F, message = F, warning = F, include = F, fig.width = 8, fig.height = 3, out.width = ".98\\textwidth", fig.cap = "State religious accommodation plots generated with the \\texttt{ggplot2} package\\citep{ggplot2}.">>=
...
p + theme(axis.text = element_blank(), axis.title = element_blank(),
  ...
  scale_colour_manual(values = rep(c("grey90", "grey20", "grey90"), 7)) #rep("black",20))
...
\includegraphics[width = .97\textwidth]{figure/state-church-plot-1}
\label{fig:state-church-plot1}
}

\subfloat[Re-imagined religious accommodation plots.] {
\includegraphics[width = .97\textwidth]{figure/state-church-plot-2}
\label{fig:state-church-plot2}
}
\caption{Panels from Michigan, Minnesota, and Mississippi, shown in original, modern, and re-imagined form.}
\end{figure}
...
\paragraph{Participants} A total of $n= 98$ participants were recruited using reddit ($n=32$) and Amazon Mechanical Turk ($n=63$). Participants were required to be over 18. We collected information about their age range, gender, and education levels. %\svp{Add in more information about demographics.}
% Add demographic information, explain difference between reddit and turk populations
