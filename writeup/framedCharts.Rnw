...
Plate 31 (see \autoref{fig:plate31})  uses spine plots to give an overview of the percentage of religious sittings by denomination for each state (colored stripes in the square) as well as the percentage of unaccommodated population over the age of ten (area of the grey outer frame),
...
Let $n_{UA}^{(\text{X})}$ be the --unfortunately unknown-- size of the unaccounted population of state {\it X}. Then we have the relationship
\begin{equation}\label{eq:nUA}
n_{UA}^{(\text{X})} = n_{\text{AGE}\ge 10}^{(\text{X})}  - n_{\text{acc}}^{(X)},
\end{equation}
where $n_{\text{AGE}\ge 10}^{(\text{X})}$ is the number of over ten year olds in the state, and $n_{\text{acc}}^{(X)}$ is the size of the population accounted for (i.e. gainfully employed or going to school, or those who are religiously accommodated). $n_{\text{acc}}^{(X)}$ is known for each state from table NT13.
We get an estimate for the unkown number of over ten year olds in each state $n_{\text{AGE}\ge 10}^{(\text{X})}$ from:
\[
...
\[
\widehat{p_{\text{AGE}\ge 10}}^{(\text{X})} = \frac{\text{\# individuals age ten and over in state } X}{\text{\# individuals in state } X}.
\]
...
\widehat{n_{UA}}^{(\text{X})} &=& n_\text{total}^{(\text{X})} \cdot \widehat{p_{\text{AGE}\ge 10}}^{(\text{X})}  - n_{\text{acc}}^{(X)}, \\ \label{eq:seUA}
s.e.(\widehat{n_{UA}}^{(\text{X})})^2 &=& \widehat{p_{\text{AGE}\ge 10}} \cdot \left( 1- \widehat{p_{\text{AGE}\ge 10}}\right) n_\text{total}^{(\text{X})}.
\end{eqnarray}
...
Figure~\ref{fig:plate32} shows a miniature of the chart published as plate \#32 in the Statistical Atlas of 1874 \citep{atlas} produced from data collected in the 9th US Census. The chart shows the gender ratio of population over the age of ten in different types of occupations.
...
\includegraphics[width=.14\textwidth,valign=c]{figure/overall-work-mosaic-legend-1}
  ...
  \subfloat[US-wide overview with integrated unemployed population.]{
...
With the help of the description (Figure~\ref{fig:header}) and the legend (Figure~\ref{fig:legend}), we can interpret the details of each of the squares in Figure~\ref{fig:us}. This figure shows an overview of type of occupation by gender across the US in 1870. The percentage of population in a particular type of occupation is shown as the widths of the rectangles, the heights are proportional to the percentage of gender, with men shown in the bottom rectangle and women in the top rectangle. The grey band around each one of the states' squares is proportional to the size of the ``unaccounted" for population, i.e.\ the difference between the total population over the age of ten and the population gainfully employed in one of the five categories or attending school.
...
This figure reveals another previously hidden finding: about 97\% of the unaccounted population are women and girls!
...
\begin{compactitem}
\item[\bf Top four only:] A decision was made to only show the top four denominations in each state and the top eight denominations overall. As a result, some denominations are shown which have a lower overall number of sittings, while other denominations are excluded even though they have more sittings. For example,  in 1870 the Reformed Late Germans have 431,700 sittings nation-wide; the 10th highest number, as shown in Figure \ref{fig:barchart-31}. However, they are not mentioned in plate \#32. The Mormon Church is locally (Utah Territory) strong enough to be represented, even though with 87,838 sittings (17th place nationally) it is far behind the German Reformed Church. This decision was likely due in part to technical constraints: it would be very hard to align very thin bars with even high-precision lithography, as each color layer of each chart must be drawn by hand and carefully aligned. Limiting the choices to five total internal bars ensured that it was generally possible to render the image and match the colors to the denominations listed in the legend.
...
The model is written in the form of regressions lines of the form  $\log E = \alpha + \beta \log E$, with the unframed version of each of the types as the base line. The other parameters can then be written as contrasts to the base line, producing a set of  easily interpretable results:
