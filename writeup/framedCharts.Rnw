...
percentages %>%
  ...
  theme(legend.position="none",
        aspect.ratio=1) +
  scale_colour_brewer("Plot Area", palette="Set2") +
...
We model $d_{ijk}$ as 
\begin{equation}\nolabel
d_{ijk} &=& a_{k1} + b_{k1} \cdot p  + \left(a_{kj} + b_{kj} \cdot p\right),\\
\end{equation}

Let $k = 1, 2, 3$ correspond to the three plot types, with 1 = mosaic, 2 = spine, and 3 = pie. 
...
error2b <- lm(diff.error ~ Type-1 + (frameframe-1+perc:frameframe):Type, data = percentages)
