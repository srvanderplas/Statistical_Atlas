...
<<overall-work-mosaic-unaccommodated, dependson="data", echo = FALSE, include=F, eval=TRUE, fig.cap="Recreation of the mosaicplot nation-wide overview of gender ratio by occupation based on gainfully employed population over ten.", warning = F, message=F, fig.align='center', fig.keep=TRUE, fig.width = 3.5>>=
...
@
<<overall-work-mosaic-legend, include = F, fig.width = 1.5>>=
library(grid)
grid.newpage()
...
church_noterritories %>%
  ...
  scale_x_continuous("Estimated Unaccommodated Population, 1% microsample") +
  scale_y_continuous("Estimated Unaccommodated Population, pixel values")
@
