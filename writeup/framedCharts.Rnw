...
Let $E_{ijk\ell}$ be the estimated size of the area for a piece in plot type $i$ (with 1 = mosaic, 2 = pie, and 3 = spine),  location $j$ (with 1 = piece in unframed plot, 2 = inside piece of a framed plot, and 3 = frame piece of a framed plot) and response $k$ where $k = 1, ..., n_{ij}$, where $n_{ij}$ is the number of evaluations for plot-frame location combination $ij$, and $A_{ijk\ell}$ the corresponding actual size of the piece. 
...
\begin{eqnarray*}
\log E_{ijk\ell} &=& \alpha_{i} + \beta_{i} \cdot \log A_{ijk\ell}  + \left(\alpha_{ij} + \beta_{ij} \cdot p\right) + e_{ijk} + u_\ell, \hspace{1.5cm} \text{Model (1)}
\end{eqnarray*}
where
\begin{itemize}
...
\end{itemize}
Estimates for all of the parameters are shown in table~\ref{tab:results}.
\hh{and now on to a dicussion of all of those results}
