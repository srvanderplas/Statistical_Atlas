...
percentages %>%
  ggplot(aes(x = perc, y = log(ratio), colour=frameframe)) + 
  geom_hline(yintercept=0, colour="grey50", size=0.5) +
  geom_point() +
  facet_grid(.~Type, labeller="label_both") + 
  geom_smooth(method="lm") +
...
\hh{Let $r_{ijk\ell}$ be the ratio of estimated and actual size of the questioned area ... we then model the log ratio $\log r_{ijk\ell}$. Results in the table are from the log ratio.} 
Let $d_{ijk\ell}$ be the difference between estimated and actual size of the questioned area for plot type $i$ (with 1 = mosaic, 2 = pie, and 3 = spine), piece location $j$ (with 1 = piece in unframed plot, 2 = inside piece of a framed plot, and 3 = frame piece of a framed plot) and response $k$ where $k = 1, ..., n_{ij}$, where $n_{ij}$ is the number of evaluations for plot-frame location combination $ij$. Each combinations has at least 25 responses from reddit users and 50 responses from amazon turk participants (see figure~\ref{fig:demographics} for an overview). 

\hh{The idea is to write the model in form of regressions lines of the log ratio $\log r$ by actual size $p$, with the unframed version of each of the types as base line.}
...
We model $d_{ijk\ell}$ as Model (1)
\begin{eqnarray*}
d_{ijk\ell} &=& \alpha_{i} + \beta_{i} \cdot p  + \left(\alpha_{ij} + \beta_{ij} \cdot p\right) + e_{ijk} + u_\ell,
\end{eqnarray*}
where 
\begin{itemize}
...
\item[$\beta_i$] can be interpreted as the average change in the difference between estimated and actual size of a piece in an unframed design of plot type $i$ ($i = 1, 2, 3$) when the actual size increase by 1 percentage point,  
\item[$\alpha_{ij}$, $\beta_{ij}$] are the average changes to $\alpha_i$ and $\beta_i$ for a piece in location $j$ ($j = 1,2,3$). For estimability, $\alpha_{i1} = \beta_{i1} = 0$ for all plot types $i$,
