...
\caption{\label{fig:combinations} Overview of the six plots shown to one third of the participants: all six combinations of plot types and designs are covered. The yellow \svp{`A', `B', and `C's} mark the areas that participants are asked to estimate. Three areas in each of the plots were selected for estimation. }
\end{figure}
...
Estimates for all of the \hh{fixed effects} parameters are shown in \autoref{tab:results}. Variance estimates show that performance varies between participants  ($\widehat{\sigma_u} = \Sexpr{round(sigma_u, 3)}$) but  overall variability in evaluation of sizes is dominated by the residual error $\widehat{\sigma_e} = \Sexpr{round(sigma_eps, 3)}$.
...
\item Estimation of the inside portion of framed plots is not significantly different from estimation of portions of unframed plots in \svp{mosaics or pie charts, as }both $\alpha_{\cdot 2}$ and $\beta_{\cdot 2}$ are not significantly different from the unframed plot estimates. It is possible that the known bias towards under-estimation of area is counteracted by the over-estimation of the area of the inside portion of the framed plot (and corresponding under-estimation of the framed region). 
\svp{For spine plots, estimation of the inside portion shows a larger bias towards over-estimation of small areas; that is, $\alpha_{22}$ = \Sexpr{round(xtab[15,4], 3)}. Even in framed spine plots, though, the small area over-estimation bias is smaller than in unframed mosaic or pie charts.}
\item Estimation of the framed portion of the plot is significantly worse than estimation of the same plot area in the unframed version of the plot for \svp{all three types of charts (though spine plots are only marginally significant)}. In the unframed version of mosaic and pie charts, the unaccommodated area tends to be over-estimated for small areas and approximately accurate for large areas; in the framed versions, the framed area tends to be under-estimated.
\end{itemize}
