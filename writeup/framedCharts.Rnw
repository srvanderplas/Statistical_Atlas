...
percentages %>%
  ...
  xlab("Actual Percentages") +
  ylab("Difference between observed and actual percentages") 
...
<<echo=FALSE>>=
percentages %>% mutate(
  ResponseId = reorder(ResponseId, as.numeric(as.factor(source)))
) %>% 
  ggplot(aes(x=ResponseId, y=howmuch)) + 
