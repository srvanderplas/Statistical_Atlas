...
percentages %>%
  ...
  geom_point(aes(shape= abs(log(ratio)) > 2)) +
  facet_grid(.~Type, labeller="label_both") +
  geom_smooth(method="lm") +
  theme_bw() +
  theme(legend.position="bottom",
...
percentages %>% filter(abs(ratio) < 2.5) %>%
  ggplot(aes(x = log(perc), y = log(howmuch), colour=frameframe)) +
  geom_abline(slope = 1, colour="grey50", size=0.5) +
  geom_point() +
  facet_grid(.~Type, labeller="label_both") +
...
baseplus <- lmer(log(howmuch) ~ Type-1 + (frameframe*log(perc)):Type + (1|ResponseId), data = percentages %>% filter(abs(log(ratio)) < 2))
