...
<<overall-work-mosaic-unaccommodated, dependson="data", echo = FALSE, include=T, fig.cap="Recreation of the mosaicplot nation-wide overview of gender ratio by occupation based on gainfully employed population over ten. \\label{fig:us-plate32-redo}", out.width='.75\\textwidth', fig.width=5, fig.height=6, warning = F, message=F, fig.align='center'>>=
 #cols <- c("#333300", "#000066", "#cc9900", "#202060", "#86742d", "grey40")
cols <- c("#30200c", "#000066", "#cc9900", "#4d374a", "#9e7637", "grey40")
...
<<religious-bias-plot, echo = FALSE, include=T, fig.width=5, fig.height=5, fig.caption = "Percentage of state-wide unaccommodated population based on estimates from the 1\\% Microsample ($x$-axis) and pixel values measured from the digitized version of plate \\#31 ($y$-axis).\\label{fig:matches-plate31}", out.width = ".5\\textwidth">>=
...
<<state-church-plot, dependson="code", echo = F, message = F, warning = F, include = T, fig.width = 8, fig.height = 3, out.width = ".9\\textwidth", fig.cap = "State religious accommodation plots generated with the \\texttt{ggplot2} package\\citep{ggplot2}.">>=
...
Figure~\ref{fig:check0} shows that:
\begin{itemize}
\item  estimates of frame pieces are not different from estimates of other pieces in designs without a frame
\item  pieces outside the frame have a tendency to be overestimated in framed designs
\item  pieces of the frame are underestimated in framed designs
\item several weird outliers
\end{itemize}

Because estimates of pieces in unframed designs corresponding to the frame of framed designs do not appear different from pieces inside, we only distinguish between three types of pieces and designs: pieces of an unframed plot, pieces of the frame in a framed plot and pieces within the plot of a framed plot.
...
As a measure of the error we use the difference between the estimated and the actual size of the piece. A positive difference means that the participant  overestimated the size of the piece in his or her estimate. Similarly, a negative difference corresponds to a visual underestimation of the piece.
...
percentages <- percentages %>% mutate(
  frameframe = factor(frameframe, levels=c("Unframed", "Framed-inside", "Framed-frame"))
)
percentages %>%
  ggplot(aes(x = perc, y = diff.error, colour=frameframe)) + 
  ...
  xlab("Actual Percentage of Area") +
  ylab("Difference between estimated \nand actual percentage") 
@

Figure~\ref{fig:model} shows a scatterplot of the difference in estimated and actual size of the area in the plot colored by different types of areas. We can see that for inside pieces of unframed and unframed designs participants have a tendency to overestimate the size of small areas and underestimate the size of the larger areas. For framed designs the error made in the visual estimation is much larger overall and depends on the location of the estimated area. Areas on the inside are overestimated, while areas of the frame are grossly underestimated. We can model these differences in estimated and actual area size as follows.
...
Let $d_{ijk}$ be the difference between estimated and observed difference of the questioned area for plot type $i$ (with 1 = mosaic, 2 = pie, and 3 = spine), piece location $j$ (with 1 = piece in unframed plot, 2 = inside piece of a framed plot, and 3 = frame piece of a framed plot) and response $k$ where $k = 1, ..., n_{ij}$, where $n_{ij}$ is the number of evaluations for plot-frame location combination $ij$. Each combinations has at least 25 responses from reddit users and 50 responses from amazon turk participants. See figure~\ref{fig:demographics} for an overview. 

The idea is to write the model in form of regressions lines of the difference $d$ by actual size $p$, with the unframed version of each of the types as base line. The other parameters can then be written as contrasts to the base line and be interpreted nicely.

We model $d_{ijk}$ as Model (1)
\begin{eqnarray*}
d_{ijk\ell} &=& \alpha_{i} + \beta_{i} \cdot p  + \left(\alpha_{ij} + \beta_{ij} \cdot p\right) + \epsilon_{ijk} + u_\ell,
\end{eqnarray*}
where 
\begin{itemize}
\item[$a_i$] can be interpreted as the estimated size of an area in an unframed design with an actual size of zero for each of the plot types $i$ ($i = 1, 2, 3$),
\item[$b_i$] can be interpreted as the average change in the difference between estimated and actual size of a piece in an unframed design of plot type $i$ ($i = 1, 2, 3$) when the actual size increase by 1 percentage point,  
\item[$a_{ij}$, $b_{ij}$] are the average changes to $a_i$ and $b_i$ for a piece in location $j$ ($j = 1,2,3$). For estimability, $a_{i1} = b_{i1} = 0$ for all plot types $i$,
\item[$\epsilon_{ijk}$] is the error observed within each combination of plot type and piece location $ij$. We assume $\epsilon_{ijk} \sim N(0, \sigma^2)$ i.i.d.\ for all $k=1, ..., n_{ij}$,
\item[$u_\ell$] is a random effect introduced by participant $\ell$, $\ell = 1, ..., N$, where $N = 95$ participants taking part in the study. Each participant was supposed to evaluate 18 pieces Each participants evaluated at least ten of the 18 pieces and give, on average, 17 (valid) estimations out of the 18. We assume that the random effect is normally distributed, i.e.\ $\u{\ell} \sim N(0, \sigma_u^2)$ i.i.d.\ for all $\ell=1, ..., N$,  and independent of the error, i.e.\ $cov(u, \epsilon) = 0$.
\end{itemize}

<<re_model, warning=FALSE, message=FALSE>>=
...
) %>% separate(PlotFrame, into=c("Type", "Location", "Slope"), sep=":") %>% mutate(
  Slope = replace(Slope, Location=="perc", "perc"),
  Location = replace(Location, Location=="perc", NA),
...
xtab <- xtab %>% dplyr::select(-Pr...t.., -Slope)
