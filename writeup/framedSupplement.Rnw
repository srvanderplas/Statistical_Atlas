...
createSpines <- function(data, state_name = "") {
  ...
  get_plots <- function(sub_data) {
    ...
    sub_data %>%
      ggplot() +
      geom_bar(aes(weight = Number,  x = 0.5, fill = group),
  ...
  getMultiSpline <- function(df) { # Pass in a single state's data, get out the single state's built data frame
    get_plots(df) %>%
      ggplot_build() %>%
      magrittr::extract2("data") %>%
      magrittr::extract2(1) %>%
      mutate(STATEICP = unique(df$STATEICP))
  }

  scalars <- spine_df %>%
    mutate(frame = group=="Unaccommodated") %>%
    group_by(STATEICP, frame) %>%
    summarize(
      Number = sum(Number)
    ) %>% mutate(
      weight = Number/sum(Number)
  ...
  scalarweight <- scalars %>%
    mutate(Accommodated = sqrt(Accommodated),
           Unaccommodated = 1 - Accommodated) %>%
    mutate(splineWt = Accommodated, frameWt = Unaccommodated/2) %>%
    select(-Accommodated, -Unaccommodated)

  # make inside plot, then scale
  ggp <- spine_df %>% filter(group != "Unaccommodated") %>%
    split(.$STATEICP) %>%
    map_df(getMultiSpline) %>%
    left_join(scalarweight)

  ggp_df <- ggp %>% mutate(
    xmin = xmin*splineWt + frameWt,
    xmax = xmax*splineWt + frameWt,
    ymin = ymin*splineWt + frameWt,
    ymax = ymax*splineWt + frameWt
  )

  plot1 <- ggp_df %>%
    ggplot() +
    geom_rect(xmin=0, xmax=1, ymin=0, ymax=1, fill="grey70", colour="grey40", size=.25) +
    geom_rect(aes(xmin=frameWt, xmax=1-frameWt, ymin=frameWt, ymax=1-frameWt), fill="white", color = "grey50") +
    geom_rect(aes(xmin=ymin, xmax=ymax, ymin=frameWt, ymax=1-frameWt, fill=fill, alpha=alpha),
              colour="grey85", size=0.1) +
    scale_fill_identity() +
    scale_alpha_identity() +
    xlim(c(0,1)) +
  ...
  plot2 <- get_plots(spine_df)

  if (state_name == "all") {
    plot1 <- plot1 + facet_wrap(~STATEICP, ncol = 6)
    plot2 <- plot2 + facet_wrap(~STATEICP, ncol = 6)
  } else {
    plot1 <- plot1 + xlab(state_name)
    plot2 <- plot2 + xlab(state_name)
  }
...
occ3 <- occ3 %>% mutate(
  ...
  mutate(State = str_replace_all(
    State,
    c("(Arizona|New Mexico|Utah|Colorado) Territory" = "SW Territories",
      "(Idaho|Wyoming|Washington|Montana|Dakota) Territory" = "NW Territories")))

stateorder <- unique(occ3$State)
stateorder <- c(stateorder[-which(stateorder %in% c("District of Columbia", "NW Territories", "SW Territories"))],
                c("District of Columbia", "NW Territories", "SW Territories"))
...
cl_data <- cl %>%
  mutate(STATEICP = as.character(Area.name))

stateorder <- unique(cl_data$STATEICP)
stateorder <- c(stateorder[-which(stateorder %in% c("District of Columbia", "NW Territories", "SW Territories"))],
                c("District of Columbia", "NW Territories", "SW Territories"))
cl_data$STATEICP <- factor(cl_data$STATEICP, levels = stateorder, ordered = T)
...
<<plate32-repro, dependson="data", include = T, out.width = "\\textwidth", fig.width = 7.5, fig.height = 9, warning = F, message = F, fig.cap = "Reproduction of plate \\#32.", fig.align='center'>>=
...
<<plate31-repro, dependson="data", include = T, out.width = "\\textwidth", fig.width = 7.5, fig.height = 9, warning = F, message = F, fig.cap = "Reproduction of plate \\#32.", fig.align='center'>>=
...
\autoref{fig:qualtrics-screenshot} shows one question from the Qualtrics survey. After 25 seconds showing one image, the survey automatically advances to the next question.
\begin{figure}
\centering
\includegraphics[width=.9\textwidth]{images/Area_Study_screenshot}
\caption{Screenshot from the Qualtrics survey.} \label{fig:qualtrics-screenshot}
\end{figure}
